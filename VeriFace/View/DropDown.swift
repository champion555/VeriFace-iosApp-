...
//    }
    ...
    public func showList() {
        ...
        let height = (self.parentController?.view.frame.height ?? 0) - (self.pointToParent.y - self.scrollPosition! + self.frame.height + 5)
        var y = self.pointToParent.y+self.frame.height+5 - self.scrollPosition!
        if height < (keyboardHeight+tableheightX){
            y = self.pointToParent.y - tableheightX - self.scrollPosition!
        }
